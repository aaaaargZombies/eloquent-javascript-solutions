...
SkillShareServer.prototype.save = function() {
  fs.writeFile(talkFile, JSON.stringify(this.talks));
};

SkillShareServer.prototype.load = function() {
  return new Promise((res, rej) => {
    fs.readFile(talkFile, 'utf8')
      .then(text => JSON.parse(text))
      .then(json => {
        Object.keys(json).forEach(title => {
          this.talks[title] = json[title];
        });
      })
      .then(x => res())
      .catch(e => res());
